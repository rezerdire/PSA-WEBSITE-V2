<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Registration {{ isset($idCardUrl) && $idCardUrl ? 'Approved' : 'Confirmed' }}</title>
    </head>
            ...
            <tr>
                <td align="center">
                    <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
                        ...
                        <tr align="center">
                            <td align="center" style="padding-bottom: 6px;">
                                <h1 style="margin:0;font-size:22px;font-weight:800;color:#374151;">
                                    {{ isset($idCardUrl) && $idCardUrl ? 'Registration Approved!' : 'Registration Submitted!' }}
                                </h1>
                            </td>
                        </tr>
                        <tr>
                            <td align="center" style="padding-bottom: 28px;">
                                <p style="margin:0;font-size:13px;color:#9ca3af;max-width:400px;line-height:1.6;">
                                    @if(isset($idCardUrl) && $idCardUrl)
                                        Your registration for <strong style="color:#374151;">PSA Annual Convention 2026</strong>
                                        has been approved. Your official membership ID card is below and attached to this email.
                                    @else
                                        Your registration for <strong style="color:#374151;">PSA Annual Convention 2026</strong>
                                ...
                                <table width="100%" cellpadding="0" cellspacing="0"
                                    ...
                                    @php
                                        ...
                                        $rows = [
                                            ...
                                            ['Status',     isset($idCardUrl) && $idCardUrl ? 'Approved' : 'Pending Review'],
                                        ];
                                    @endphp
                                    ...
                                    @foreach ($rows as $index => [$label, $value])
                                        <tr style="border-top: {{ $index === 0 ? 'none' : '1px solid #f9fafb' }};">
                                            ...
                                            <td style="padding:11px 20px 11px 0;font-size:13px;font-weight:600;
                                                    color:{{ $label === 'Status' && $value === 'Approved' ? '#2e7d32' : ($label === 'Status' ? '#d97706' : '#374151') }};
                                                    vertical-align:top;">
                                                {{ $value }}
                                            </td>
                                        </tr>
                        ...
                        {{-- ID Card / QR (only shown when approved and a card exists) --}}
                        @if(isset($idCardUrl) && $idCardUrl)
                            <tr>
                                <td style="padding-bottom: 16px;">
                                    <table width="100%" cellpadding="0" cellspacing="0"
                                        ...
                                        <tr>
                                            <td colspan="2"
                                                style="background-color:#2e7d32;padding:10px 20px;
                                                    font-size:11px;font-weight:700;color:white;
                                                    text-transform:uppercase;letter-spacing:0.08em;">
                                        ...
                                        <tr>
                                            <td align="center" style="padding:20px;">
                                                <img src="{{ $idCardUrl }}" alt="PSA Membership ID Card"
                                                    style="max-width:280px;width:100%;border-radius:8px;display:block;" />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td style="padding:0 20px 16px;font-size:11px;color:#9ca3af;text-align:center;">
                                                A copy is also attached to this email as a downloadable file.
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                        ...
                        @elseif($registration->status === \App\Models\Registration::STATUS_APPROVED)
                            {{-- Approved, but no ID card is available yet (e.g. generation failed) --}}
                            <tr>
                                <td style="padding-bottom: 16px;">
                                    <table width="100%" cellpadding="0" cellspacing="0"
                                ...
                                <table width="100%" cellpadding="0" cellspacing="0"
                                    ...
                                    <tr>
                                        <td style="padding:14px 18px;font-size:12px;color:#1d4ed8;line-height:1.7;">
                                            @if(isset($idCardUrl) && $idCardUrl)
                                                Please keep this ID card for your records and present it at PSA events for verification.
                                            @else
